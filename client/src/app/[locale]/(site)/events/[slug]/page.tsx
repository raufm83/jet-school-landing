...
export async function generateMetadata({ params }: ISinglePostPageProps): Promise<Metadata> {
  try {
    ...
    const canonicalUrl = buildHreflangUrl(baseUrl, locale, `events/${locale === "az" ? azSlug : ruSlug}`);
    ...
    return {
      ...
      alternates: {
        ...
        languages: {
          az: buildHreflangUrl(baseUrl, "az", `events/${azSlug}`),
          ru: buildHreflangUrl(baseUrl, "ru", `events/${ruSlug}`),
          "x-default": buildHreflangUrl(baseUrl, "az", `events/${azSlug}`),
        },
      },
      openGraph: {
        title,
