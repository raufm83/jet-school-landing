...
export async function generateMetadata({ params }: ISinglePostPageProps): Promise<Metadata> {
  try {
    ...
    const canonicalUrl = ensureTrailingSlash(`${baseUrl}/${locale}/blog/${params.slug}`);
    ...
    return {
      ...
      alternates: {
        ...
        languages: {
          az: data.slug.az ? ensureTrailingSlash(`${baseUrl}/az/blog/${azSlug}`) : undefined,
          ru: data.slug.ru ? ensureTrailingSlash(`${baseUrl}/ru/blog/${ruSlug}`) : undefined,
          "x-default": data.slug.az ? ensureTrailingSlash(`${baseUrl}/az/blog/${azSlug}`) : undefined,
        },
      },
      openGraph: {
        title,
